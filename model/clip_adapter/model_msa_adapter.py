...
class ResidualAttentionBlock(nn.Module):
    def __init__(self, d_model: int, n_head: int, attn_mask: torch.Tensor = None, scale=1., num_tadapter=1, drop_path=0.):
        ...
        self.n_head = n_head
        self.MLP_Adapter = Adapter(d_model, skip_connect=True)
        self.Adapter = Adapter(d_model, skip_connect=True)
        self.scale = scale
    ...
    def forward(self, x: torch.Tensor, layer_idx=None):
        ## x shape [HW+1, BT, D]
        n, bs, d = x.shape

        # if layer_idx is None or layer_idx < 7:
        if layer_idx is None or layer_idx % 2 == 0:
...
class VisionTransformer(nn.Module):
    def __init__(self, h_resolution: int, w_resolution: int, patch_size: int, stride_size: int, width: int, layers: int, heads: int, output_dim: int, drop_path_rate, num_tadapter=1, adapter_scale=0.5):
        super().__init__()
    ...
    def forward(self, x: torch.Tensor, cv_emb = None):
        ...
        for i in range(11):
            x = self.transformer.resblocks[i](x, i+1) 
        x11 = x
        x12 = self.transformer.resblocks[11](x11, 12) 
    ...
    def __init__(self,
        ...
        if isinstance(vision_layers, (tuple, list)):
            ...
            self.visual = VisionTransformer(
                ...
                layers=vision_layers,
                heads=vision_heads,
                output_dim=embed_dim, num_tadapter=num_tadapter, adapter_scale=adapter_scale, drop_path_rate=drop_path_rate
            )
