...
class VisionTransformer(nn.Module):
    def __init__(self, h_resolution: int, w_resolution: int, patch_size: int, stride_size: int, width: int, layers: int, heads: int, output_dim: int):
        ...
        self.adapter_1 = Adapter(768, 2)
        self.adapter_2 = Adapter(768, 2)
        self.adapter_3 = Adapter(768, 2)
        self.adapter_4 = Adapter(768, 2)
        self.adapter_5 = Adapter(768, 2)
        self.adapter_6 = Adapter(768, 4)
        self.adapter_7 = Adapter(768, 4)
        self.adapter_8 = Adapter(768, 4)
        self.adapter_9 = Adapter(768, 4)
        self.adapter_10 = Adapter(768, 4)
        self.adapter_11 = Adapter(768, 4)
        self.adapter_12 = Adapter(768, 4)
        self.adapter_t = Adapter(512, 4)
    ...
    def forward(self, x: torch.Tensor, cv_emb = None):
        ...
        x1 = self.transformer.resblocks[0](x) 
        # x1 = self.adapter_1(x1)
        x2 = self.transformer.resblocks[1](x1) 
        # x2 = self.adapter_2(x2)
        x3 = self.transformer.resblocks[2](x2) 
        # x3 = self.adapter_3(x3)
        x4 = self.transformer.resblocks[3](x3) 
        # x4 = self.adapter_4(x4)
        x5 = self.transformer.resblocks[4](x4) 
        # x5 = self.adapter_5(x5)
        x6 = self.transformer.resblocks[5](x5) 
        # x6 = self.adapter_6(x6)
        x7 = self.transformer.resblocks[6](x6) 
        x7 = self.adapter_7(x7)
        x8 = self.transformer.resblocks[7](x7) 
        x8 = self.adapter_8(x8)
        x9 = self.transformer.resblocks[8](x8) 
        x9 = self.adapter_9(x9)
        x10 = self.transformer.resblocks[9](x9) 
        x10 = self.adapter_10(x10)
        x11 = self.transformer.resblocks[10](x10) 
        x11 = self.adapter_11(x11)
        ...
        x12 = self.adapter_12(x12)

        if self.proj is not None:
            xproj = x12 @ self.proj

        xproj = self.adapter_t(xproj)
...
class CLIP(nn.Module):
    def __init__(self,
                 ...
                 h_resolution: int, 
                 w_resolution: int, drop_path_rate=0.1, num_tadapter=1, adapter_scale=0.5
                 ):
        super().__init__()
...
def build_model(state_dict: dict, h_resolution: int, w_resolution: int, vision_stride_size: int, drop_path_rate=0.2, adapter_scale=0.2):
    ...
    model = CLIP(
        ...
        h_resolution, w_resolution, drop_path_rate=drop_path_rate, adapter_scale=adapter_scale
    )
    if vit:
