...
class ResidualAttentionBlock(nn.Module):
    def __init__(self, d_model: int, n_head: int, attn_mask: torch.Tensor = None, scale=0.1, drop_out=0., peft=False):
        ...
        if GLOBAL_CONFIG.MODEL.MLP_ADAPTER:
            self.MLP_Adapter = Adapter(d_model, GLOBAL_CONFIG.MODEL.MLP_ADP_MID_DIM, drop_out, scale, learnable_scalar=GLOBAL_CONFIG.MODEL.LEARNABLE_SCALAR)
    ...
    def attention(self, q: torch.Tensor, k: torch.Tensor, v: torch.Tensor, q_lora=None, k_lora=None, v_lora=None, attnmap_vis=False):
        self.attn_mask = self.attn_mask.to(dtype=q.dtype, device=q.device) if self.attn_mask is not None else None
        if attnmap_vis:
            return self.attn(q, k, v, need_weights=attnmap_vis, attn_mask=self.attn_mask, 
                         q_lora=q_lora, k_lora=k_lora, v_lora=v_lora)
        else:
            return self.attn(q, k, v, need_weights=False, attn_mask=self.attn_mask, 
                         q_lora=q_lora, k_lora=k_lora, v_lora=v_lora)[0]

    def forward(self, x: torch.Tensor, layer_idx=None, peft_stage=True, attnmap_vis=False):
        ## x shape [HW+1, BT, D]
        n, bs, d = x.shape
        ...
        if layer_idx is None or layer_idx < 0 or not GLOBAL_CONFIG.MODEL.MLP_ADAPTER or not peft_stage:
            xn = self.ln_1(x)
            if attnmap_vis:
                x_attn, attn = self.attention(xn, xn, xn, attnmap_vis=attnmap_vis)
                x = x + x_attn
            else:
            ...
            xn = self.ln_1(x)
            if attnmap_vis:
                x_attn, attn = self.attention(xn, xn, xn, x_q, x_k, x_v, attnmap_vis=attnmap_vis)
                x = x + x_attn
            else:
                x = x + self.attention(xn, xn, xn, x_q, x_k, x_v)

            x_mlp = self.mlp(self.ln_2(x))
            x = self.MLP_Adapter(x, x_mlp) + x
...
class VisionTransformer(nn.Module):
    ...
    def forward(self, x: torch.Tensor, cv_emb = None, peft_stage=True, attnmap_vis=False):
        ...
        for i in range(11):
            ...
            if attnmap_vis:
                x, attn = self.transformer.resblocks[i](x, i+1, peft_stage, attnmap_vis) 
                attn_mat.append(attn)
            else:
                x = self.transformer.resblocks[i](x, i+1, peft_stage, attnmap_vis) 
        x11 = x
        
        if attnmap_vis:
            x12, attn = self.transformer.resblocks[11](x11, 12, peft_stage, attnmap_vis)
            attn_mat.append(attn)
        else:
            x12 = self.transformer.resblocks[11](x11, 12, peft_stage, attnmap_vis)
        ...
        return x11, x12, xproj, attn_mat # [bs, 129, 768] [bs, 129, 768] [bs, 129, 512]
